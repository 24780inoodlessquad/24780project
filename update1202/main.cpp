...
int main(void)
{
	...
	Bubble player1_bubble(0, 1, p1.getY()/50, p1.getX()/50, 30);
	Bubble player2_bubble(0, 2, p2.getY()/50, p2.getX()/50, 30);
	...
	for (;;)
	{
		...
		p2.Draw(2);
		t2 = time(NULL);
		Menu1.draw(p1.Getlife(), p2.Getlife(), p1.GetBubble(), p2.GetBubble(), p1.GetRange(), p2.GetRange(), p1.GetSpeed(), p2.GetSpeed(), t2 - t1);
		FsSwapBuffers();
		FsSleep(15);
